...
class SmilarityScoreOutput(BaseModel):
    
    similarity_score: int = Field(
        description="How similar is the reponse to the examples?",
        gt = 0,
        lt = 100, 
    )
...
def judge_response(response:str, expected_category:str):

    s_llm = llm.with_structured_output(SmilarityScoreOutput)

    examples = RESPONSE_EXAMPLES[expected_category]
    result = s_llm.invoke(f"""
...
def test_individual_nodes():
    ...
    result = graph.nodes["draft_response"].invoke({"category": "spam", "email":"I have a great offer for you!", "priority_score": 1})
    similarity_score = judge_response(result["response"], "spam")
    assert similarity_score >= 70
   #assert judge_response(result["response"], "spam") == 1
